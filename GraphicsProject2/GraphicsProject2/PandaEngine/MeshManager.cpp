...
void MeshManager::DrawTransformBox()
{
    ...
    {
        ImGui::Text("Texture:", selectedMesh->texture[i]); ImGui::SameLine(); ImGui::SetNextItemWidth(100);
    }
...
void MeshManager::LoadSavedMeshes(unsigned int shaderProgramID)
{
    ...
    for (size_t i = 0; i < meshes.size(); i++)
    {
        ...
        mesh->texture[0] = meshes[i]->texture[0];
        mesh->texture[1] = meshes[i]->texture[1];
        mesh->texture[2] = meshes[i]->texture[2];
        mesh->texture[3] = meshes[i]->texture[3];
        mesh->textureRatio[0] = meshes[i]->textureRatio[0];
        mesh->textureRatio[1] = meshes[i]->textureRatio[1];
        mesh->textureRatio[2] = meshes[i]->textureRatio[2];
        mesh->textureRatio[3] = meshes[i]->textureRatio[3];
        mesh->maskTexture = meshes[i]->maskTexture;
        mesh->heightMap = meshes[i]->heightMap;
        mesh->heightMapScale = meshes[i]->heightMapScale;
        mesh->useHeightMap = meshes[i]->useHeightMap;
        mesh->isSkyBox = meshes[i]->isSkyBox;
    }
}
...
void MeshManager::SetUpTextures(cMesh* pCurrentMesh, GLuint shaderProgramID)
{
    ...
    if (!pCurrentMesh->maskTexture.empty())
    {
        ...
        glUniform1i(mask_UL, textureUnitNumber);
    }
    else
    {
        GLint maskBool_UL = glGetUniformLocation(shaderProgramID, "hasMask");
    ...
    if (pCurrentMesh->useHeightMap)
    {
        GLint heightMapBool_UL = glGetUniformLocation(shaderProgramID, "bUseHeightMap");
        glUniform1f(heightMapBool_UL, (GLfloat)GL_TRUE);

        GLint heightMapScale_UL = glGetUniformLocation(shaderProgramID, "heightScale");
        glUniform1f(heightMapScale_UL, (GLfloat)pCurrentMesh->heightMapScale);

        GLint textureUnitNumber = 30;
        GLuint heightMapID = textureManager->getTextureIDFromName(pCurrentMesh->heightMap);
        glActiveTexture(GL_TEXTURE0 + textureUnitNumber);
        glBindTexture(GL_TEXTURE_2D, heightMapID);
        GLint texture_30_UL = glGetUniformLocation(shaderProgramID, "heightMapSampler");
        glUniform1i(texture_30_UL, textureUnitNumber);
    }
    else
    {
        GLint heightMapBool_UL = glGetUniformLocation(shaderProgramID, "bUseHeightMap");
        glUniform1f(heightMapBool_UL, (GLfloat)GL_FALSE);
    }
