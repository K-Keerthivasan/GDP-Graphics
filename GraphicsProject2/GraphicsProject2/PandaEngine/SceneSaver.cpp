...
void SceneSaver::SaveMeshes(std::vector<cMesh*> meshes)
{
    ...
    if (file.is_open())
    {
        for (cMesh* mesh : meshes)
        {
            ...
            file << "Mask:" << mesh->maskTexture << "\n";
            file << "HeightMap:" << mesh->heightMap << "\n";
            file << "HeightMapScale:" << mesh->heightMapScale << "\n";
            file << "UseHeightMap:" << mesh->useHeightMap << "\n";
            file << "IsSkybox:" << mesh->isSkyBox << "\n";
...
std::vector<cMesh*> SceneSaver::LoadMeshes()
{
    ...
    if (file.is_open()) {
            ...
            if (std::getline(iss, token, ':'))
            {
                if (token == "Mesh Name")
                {
                    ...
                    if (readMeshData)
                    {
                        ...
                        newMesh->heightMap = currentMesh.heightMap;
                        newMesh->heightMapScale = currentMesh.heightMapScale;
                        newMesh->useHeightMap = currentMesh.useHeightMap;
                        newMesh->isSkyBox = currentMesh.isSkyBox;
                ...
                {
                    std::getline(iss, currentMesh.heightMap);
                }
                else if (token == "HeightMapScale")
                {
                    iss >> currentMesh.heightMapScale;
                }
                else if (token == "UseHeightMap")
                {
                    int useHeightMap;
                    iss >> useHeightMap;
                    currentMesh.useHeightMap = useHeightMap == 1;
                }
                ...
                else if (token == "IsSkybox")
                {
                    int isSkybox;
                    iss >> isSkybox;
                    currentMesh.isSkyBox = isSkybox == 1;
                }
            }
            else if (line.empty())
            {
        ...
        if (readMeshData)
        {
            ...
            newMesh->heightMap = currentMesh.heightMap;
            newMesh->heightMapScale = currentMesh.heightMapScale;
            newMesh->useHeightMap = currentMesh.useHeightMap == 1;
            newMesh->isSkyBox = currentMesh.isSkyBox;
            newMesh->texture[0] = currentMesh.texture[0];
            newMesh->texture[1] = currentMesh.texture[1];
            newMesh->texture[2] = currentMesh.texture[2];
